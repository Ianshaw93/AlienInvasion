...
def update_bullets(ai_settings, screen, ship, aliens, bullets):
    bullets.update()
    # Delete bullets once off the screen
...
def update_screen(ai_settings, screen, ship, aliens, bullets):
    ...
    ship.blitme()
    # draw all aliens in the group onto the screen
    aliens.draw(screen)

    # show most recently refreshed screen
    pygame.display.flip()

def get_number_aliens_x(ai_settings, alien_width):
    # Calulate number of aliens per row
...
def get_number_rows(ai_settings, ship_height, alien_height):
    # calc number of rows of aliens to fit in screen
    available_space_y = (ai_settings.screen_height - (ship_height + 8 * alien_height))
...
def create_fleet(ai_settings, screen, ship, aliens):
    ...
    # spacing between aliens equal to one alien width
    alien = Alien(ai_settings, screen)
...
def change_fleet_direction(ai_settings, aliens):
    # drop down fleet and change direction
    for alien in aliens.sprites():
...
def update_aliens(ai_settings, stats, screen, ship, aliens, bullets):
    ...
    # then update positions of all aliens in fleet
    check_fleet_edges(ai_settings, aliens)
    aliens.update()

    # look for aliens hitting the bottom of the screen
    check_aliens_bottom(ai_settings, stats, screen, ship, aliens, bullets)
    # scope for aline ship collisons
